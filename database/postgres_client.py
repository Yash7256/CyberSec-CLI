...
class PostgresClient:
    ...
    async def create_scan(self, target: str, user_id: Optional[str] = None, config: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        try:
            async with self.pool.acquire() as conn:
                await conn.execute(
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                    ''',
                    scan_id, target, user_id, json.dumps(config) if config else None, 'pending'
                )
            return scan_id
        except Exception as e:
    ...
    async def update_scan_status(self, scan_id: str, status: str, completed_at: Optional[datetime] = None):
        """
        Update scan status.
    ...
    async def list_user_scans(self, user_id: str, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
        """
