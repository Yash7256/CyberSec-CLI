    ...
    try:
        ...
        for scan in scans:
            c.execute(
                ...
                (scan['timestamp'], scan['target'], scan['ip'], scan['command'], scan['output'])
            )
        conn.commit()
    ...
    try:
        ...
        for i, scan in enumerate(scans):
            ...
            if 'open_ports' in output_data:
                await postgres_client.save_scan_results(scan_id, output_data['open_ports'])
        insert_time = time.time() - start_time
...
async def run_benchmark():
    ...
    for size in test_sizes:
        ...
        # Compare performance
        if (HAS_POSTGRES and 
            'error' not in sqlite_results and 
            'error' not in postgres_results):
            print("\nPerformance Comparison:")
            insert_ratio = sqlite_results['insert_time'] / postgres_results['insert_time'] if postgres_results['insert_time'] > 0 else 0
            select_ratio = sqlite_results['select_time'] / postgres_results['select_time'] if postgres_results['select_time'] > 0 else 0
