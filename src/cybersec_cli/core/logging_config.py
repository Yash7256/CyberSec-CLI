...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        log_entry = {
            ...
            "trace_id": getattr(
                record, "trace_id", getattr(_local, "request_id", str(uuid.uuid4()))
            ),
        }
...
def setup_logging(log_dir: str = "logs", audit_log_file: str = "monitoring/audit.log"):
    ...
    try:
        ...
        scanner_handler = CompressedRotatingFileHandler(
            str(scanner_log_path), maxBytes=100 * 1024 * 1024, backupCount=30  # 100MB
        )
        ...
        print(
            f"Warning: Cannot create scanner log file at {scanner_log_path}. Using console only."
        )
    ...
    try:
        ...
        api_handler = CompressedRotatingFileHandler(
            str(api_log_path), maxBytes=100 * 1024 * 1024, backupCount=30  # 100MB
        )
        ...
        print(
            f"Warning: Cannot create API log file at {api_log_path}. Using console only."
        )
    ...
    try:
        ...
        celery_log_path.parent.mkdir(
            parents=True, exist_ok=True
        )  # Ensure directory exists
        celery_handler = CompressedRotatingFileHandler(
            str(celery_log_path), maxBytes=100 * 1024 * 1024, backupCount=30  # 100MB
        )
        ...
        print(
            f"Warning: Cannot create Celery log file at {celery_log_path}. Using console only."
        )
    ...
    try:
        ...
        db_handler = CompressedRotatingFileHandler(
            str(db_log_path), maxBytes=100 * 1024 * 1024, backupCount=30  # 100MB
        )
        ...
        print(
            f"Warning: Cannot create Database log file at {db_log_path}. Using console only."
        )
    ...
    try:
        ...
        print(
            f"Warning: Cannot create audit log file at {audit_log_file}. Using console only."
        )
...
def log_audit_event(event_type: str, message: str, **context):
    ...
    audit_logger.info(
        message,
        extra={"component": "audit", "context": {"event_type": event_type, **context}},
    )
...
def add_request_context(**context):
    ...
    return {"trace_id": current_trace_id, **context}
