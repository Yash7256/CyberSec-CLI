...
async def identify_service_async(
    ...
    Runs all probes in parallel for faster detection.
    ...
    """
    best_result = {"service": None, "version": None, "banner": None, "confidence": 0.0}
    
    tasks = []
    
    # helper for running a single probe key context
    async def try_probe(service_name, probe_data):
        try:
            response = await send_async_probe(ip, port, probe_data, timeout)
            if response:
                confidence = _analyze_response(service_name, response)
                if confidence > 0:
            ...
            logger.debug(f"Async probe failed for {service_name} on {ip}:{port} - {e}")
        return None

    # Launch all probes
    for service_name, probes in SERVICE_PROBES.items():
        for probe in probes:
            tasks.append(try_probe(service_name, probe))
            
    if not tasks:
        return best_result
        
    # Wait for all probes to complete
    # In the future, we could race them and cancel others if we get a high confidence match
    ...
    if best_result["confidence"] == 0.0:
        port_service = _get_service_by_port(port)
        if port_service:
            best_result["service"] = port_service
            best_result["confidence"] = 0.3  # Low confidence for port-based detection
