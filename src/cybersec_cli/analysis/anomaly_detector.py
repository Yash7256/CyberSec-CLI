...
class MLAnomalyDetector:
    ...
    def __init__(self, model_path: str = None):
        ...
        self.feature_names = [
            ...
            'error_in', 'error_out', 'drop_in', 'drop_out', 'connections'
        ]
        self.model_path = model_path or str(MODEL_SAVE_PATH / 'anomaly_detection_model.joblib')
        self.load_model()
        ...
        for metric, value in metrics.items():
            if self.baseline and metric in self.baseline.get('percentiles', {}):
                ...
                if value > p99 * 2:  # Value exceeds 2x 99th percentile
                    score = (value - self.baseline['mean']) / (self.baseline['std'] or 1.0)
...
class NetworkAnomalyDetector(AnomalyDetector):
    ...
    def __init__(self, interface: str = None, use_ml: bool = True):
        ...
        self.connections: Dict[tuple, NetworkConnection] = {}
        self.port_activity = defaultdict(lambda: {'count': 0, 'last_seen': 0, 'first_seen': 0})
        self.protocol_stats = defaultdict(lambda: {'count': 0, 'bytes': 0, 'connections': set()})
    ...
    def _get_protocol(self, port: int) -> Protocol:
        ...
        if port in {20, 21, 22, 23, 25, 53, 80, 110, 143, 443, 587, 993, 995, 3306, 3389, 5432, 8080, 8443}:
            return Protocol.TCP
    ...
    def _detect_port_scan(self) -> List[Anomaly]:
        ...
        for ip, connections in scan_candidates.items():
            if len(connections) > 5:  # Threshold for scan detection
                ...
                time_window = max(c['time'] for c in connections) - min(c['time'] for c in connections)
            ...
            if port_connections > 50 and time_window < 30:  # 50+ connections in 30s
                ...
                anomalies.append(Anomaly(
                    ...
                    metadata={
                        ...
                        'connections_per_second': port_connections / (time_window or 1)
                    }
                ))
    ...
    def _detect_connection_anomalies(self) -> List[Anomaly]:
        ...
        connections_by_host = defaultdict(lambda: {
            'connections': [],
            'ports': set(),
            'protocols': set(),
        ...
        for host, data in connections_by_host.items():
            ...
            is_private = host.startswith(('127.', '10.', '192.168.', '172.16.')) or host == '::1'
            ...
            if not is_private and time_window > 0 and (conn_count / time_window) > 5:  # >5 connections/second
                anomalies.append(Anomaly(
                    anomaly_type=AnomalyType.SECURITY_ALERT,
                    timestamp=datetime.now(),
                    score=8.5,
    ...
    def analyze_traffic(self) -> List[Anomaly]:
        ...
        rates = {
            ...
            'bytes_sent_rate': (current_metrics.bytes_sent - self.last_metrics.bytes_sent) / time_diff,
            'bytes_recv_rate': (current_metrics.bytes_recv - self.last_metrics.bytes_recv) / time_diff,
            'packets_sent_rate': (current_metrics.packets_sent - self.last_metrics.packets_sent) / time_diff,
            'packets_recv_rate': (current_metrics.packets_recv - self.last_metrics.packets_recv) / time_diff,
            'connections': current_metrics.connections,
            'error_rate': ((current_metrics.error_in + current_metrics.error_out) - 
                          (self.last_metrics.error_in + self.last_metrics.error_out)) / time_diff,
            'drop_rate': ((current_metrics.drop_in + current_metrics.drop_out) - 
                         (self.last_metrics.drop_in + self.last_metrics.drop_out)) / time_diff
        }
        ...
        if hasattr(current_metrics, 'connection_stats'):
            rates.update({
                ...
                'established_connections': current_metrics.connection_stats['by_status'].get('ESTABLISHED', 0),
                'time_wait_connections': current_metrics.connection_stats['by_status'].get('TIME_WAIT', 0),
                'tcp_connections': current_metrics.connection_stats['by_protocol'].get(Protocol.TCP, 0),
                'udp_connections': current_metrics.connection_stats['by_protocol'].get(Protocol.UDP, 0),
                'icmp_connections': current_metrics.connection_stats['by_protocol'].get(Protocol.ICMP, 0)
            })
        ...
        if hasattr(current_metrics, 'connection_stats'):
            for proto, count in current_metrics.connection_stats['by_protocol'].items():
                if proto == Protocol.UNKNOWN and count > 5:  # Many unknown protocol connections
                    anomalies.append(Anomaly(
        ...
        if rates['error_rate'] > 10:  # More than 10 errors per second
            anomalies.append(Anomaly(
                ...
                metadata={
                    ...
                    'errors_in': current_metrics.error_in - self.last_metrics.error_in,
                    'errors_out': current_metrics.error_out - self.last_metrics.error_out
                }
            ))
    ...
    def _detect_anomalies(self, current_rates: Dict[str, float]) -> List[Anomaly]:
        ...
        if 'connections' in current_rates:
            ...
            if conn_history:
                ...
                std_conn = (sum((x - mean_conn) ** 2 for x in conn_history) / len(conn_history)) ** 0.5 or 1.0
    ...
    def analyze_logs(self, error_count: int, failed_login_count: int) -> List[Anomaly]:
        ...
        if len(self.error_rates) >= 5:  # Minimum samples for baseline
            ...
            if error_anomaly:
                ...
                error_anomaly.description = f"Unusual error rate detected: {error_count} errors"
            ...
            if login_anomaly:
                ...
                login_anomaly.description = f"Suspicious login activity: {failed_login_count} failed attempts"
