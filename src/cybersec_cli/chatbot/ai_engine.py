...
class AIEngine:
    ...
    def __init__(self, api_key: str = None, model: str = None):
        ...
        if not self.use_api:
            logger.info("OpenAI API not configured. Using fallback rule-based analysis.")
    ...
    async def generate_response(
        ...
        if self.use_api:
            return await self._generate_api_response(messages, tools, temperature, max_tokens)
        else:
    ...
    def _analyze_message(self, message: str) -> str:
        ...
        if any(service in message_lower for service in ["ssh", "http", "ftp", "mysql", "postgres", "redis"]):
            return self._analyze_service_security(message)
        
        # General security question
        if any(keyword in message_lower for keyword in ["secure", "risk", "vulnerability", "safe", "threat"]):
    ...
    def _analyze_port_security(self, message: str) -> str:
        ...
        port_patterns = {
            "22": ("SSH", "Remote access", "Critical - Control access with firewall rules"),
            "80": ("HTTP", "Web server", "Use HTTPS instead (port 443)"),
            "443": ("HTTPS", "Secure web", "Generally secure if TLS is properly configured"),
            "3306": ("MySQL", "Database", "CRITICAL - Never expose database to internet"),
            "5432": ("PostgreSQL", "Database", "CRITICAL - Keep on internal network only"),
            "6379": ("Redis", "Cache", "CRITICAL - Expose only to internal services"),
            "27017": ("MongoDB", "Database", "CRITICAL - Restrict to private network"),
            "8080": ("HTTP Alt", "Web proxy/app", "Verify if necessary, consider firewall"),
        }
    ...
    def _analyze_service_security(self, message: str) -> str:
        ...
        services = {
            "ssh": {
                ...
                "risks": ["Brute force attacks", "Weak authentication", "Exposed to internet"],
                "recommendations": ["Use key-based auth", "Disable root login", "Change default port", "Fail2ban"]
            },
            "http": {
                "description": "Hypertext Transfer Protocol",
                "risks": ["Unencrypted transmission", "Man-in-the-middle attacks", "Data exposure"],
                "recommendations": ["Use HTTPS instead", "Redirect HTTP to HTTPS", "Enable HSTS header"]
            },
            "mysql": {
                "description": "MySQL Database",
                "risks": ["Database exposure", "SQL injection", "Unauthorized access"],
                "recommendations": ["Restrict to private network", "Strong passwords", "Use firewall", "Regular backups"]
            },
            "ftp": {
                "description": "File Transfer Protocol",
                "risks": ["Unencrypted credentials", "Plain text data", "Outdated protocol"],
                "recommendations": ["Use SFTP instead", "Disable FTP", "Use SSH/SCP", "Never expose to internet"]
            },
            "postgres": {
                "description": "PostgreSQL Database",
                "risks": ["Network exposure", "Brute force", "Default credentials"],
                "recommendations": ["Private network only", "Strong passwords", "Firewall rules", "SSL connections"]
            },
            "redis": {
                "description": "Redis Cache",
                "risks": ["No authentication by default", "Data exposure", "Network attacks"],
                "recommendations": ["Internal network only", "Enable AUTH", "Use firewall", "Strong passwords"]
            }
        }
    ...
    def _generate_security_advice(self, message: str) -> str:
        ...
        response += "     - Use multiple layers of security (firewall, auth, encryption)\n\n"
    ...
    def _generate_default_response(self, message: str) -> str:
        ...
        response += "Set OPENAI_API_KEY environment variable to enable GPT-4 analysis.\n"
    ...
    async def stream_response(
        ...
        if self.use_api:
            async for chunk in self._stream_api_response(messages, tools, temperature, max_tokens):
                yield chunk
        else:
            # For fallback, yield the response as if it were streaming
            response = await self.generate_response(messages, tools, temperature, max_tokens)
