...
class GroqClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        Args:
            api_key: Groq API key. If not provided, looks for GROQ_API_KEY env var.
        """
    ...
    async def chat_completion(self, messages: List[Dict[str, str]], temperature: float = 0.7) -> Dict[str, Any]:
        """
        Send a chat completion request to Groq API asynchronously.
        ...
        def _perform_request():
            try:
                ...
                raise Exception(f"Groq API Error {e.code}: {e.reason}") from e
