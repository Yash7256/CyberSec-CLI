...
class RateLimitConfig(BaseModel):
    ...
    rate_limit_whitelist: list = Field(default_factory=list)  # IPs with unlimited access
...
class CORSConfig(BaseModel):
    """CORS configuration."""
    allow_origins: list = Field(default_factory=lambda: ["http://localhost", "http://localhost:3000", "http://127.0.0.1", "http://127.0.0.1:3000"])
