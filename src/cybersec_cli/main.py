...
class CyberSecCLI:
    ...
    def update_footer(self, message: str = ""):
        ...
        status = " | ".join(context_info) if context_info else "Type 'help' for commands"
        if message:
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    self.running = False
                    self.layout["content"].update(
                        Panel("[yellow]Exiting Cybersec CLI. Stay secure![/]", title="[bold]Exit[/]")
                    )
                    live.refresh()
                    break
    ...
    async def process_command(self, command: str):
        ...
        try:
            ...
            if cmd in ('exit', 'quit'):
                ...
                self.layout["content"].update(
                    Panel(Text.from_markup("[yellow]Exiting Cybersec CLI. Stay secure![/]"))
                )
            elif cmd in ('help', '?'):
                self.show_help()
            elif cmd == 'clear':
                self.layout["content"].update(
                    Panel(Text.from_markup("Cleared. Type 'help' for available commands."))
                )
            elif cmd == 'scan':
                if not args:
                    self.layout["content"].update(
                        Panel(Text.from_markup("[yellow]Please specify a target to scan.\nExample: scan example.com[/]"))
                    )
                else:
    ...
    async def handle_scan_command(self, args: str):
        ...
        if not parts:
            self.layout["content"].update(
                Panel(Text.from_markup("[yellow]Please specify a target to scan. Example: scan example.com[/]"))
            )
            return
        ...
        if '-p' in parts:
            try:
                ...
                self.layout["content"].update(
                    Panel(Text.from_markup("[red]Invalid port specification. Use: -p PORT, -p START-END, or -p PORT1,PORT2,...[/]"))
                )
                return
        
        # Update UI to show scanning has started
        self.layout["content"].update(
            Panel(Text.from_markup(f"[yellow]Scanning {target}... This may take a moment.[/]"))
        )
        ...
        try:
            ...
            open_ports = sorted([r for r in results if r.state.value == "open"], key=lambda x: x.port)
            ...
            for severity, emoji in severity_emojis.items():
                if findings_by_severity[severity]:
                    ...
                    for finding in findings_by_severity[severity]:
                        ...
                        recommendations = [r.replace('1. ', '• ') if r.startswith('1. ') else f"• {r}" for r in recommendations]
            ...
            for finding in sorted_findings:
                severity = finding["severity"]
                color = {"High": "red", "Medium": "yellow", "Low": "cyan"}.get(severity, "dim")
                ...
                tech_details.add_row("🎯 [bold]CVSS Vector:[/]", f"[dim]{cvss.get('vector', 'N/A')}[/]")
                tech_details.add_row("💫 [bold]Impact:[/]", f"{cvss.get('impact', 'N/A'):.1f}")
                tech_details.add_row("⚡ [bold]Exploitability:[/]", f"{cvss.get('exploitability', 'N/A'):.1f}")
                ...
                # CWE and Compliance
                tech_details.add_row("", "")  # Spacer
                tech_details.add_row("📋 [bold]CWE ID:[/]", finding.get("cwe_id", "N/A"))
            ...
            self.layout["content"].update(
                Panel(f"[red]Error during scan: {str(e)}[/]")
            )
