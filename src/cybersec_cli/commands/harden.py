...
def check_root():
    ...
    if os.geteuid() != 0:
        console.print("[bold red]Error: This command requires root privileges. Please run with sudo.[/bold red]")
        sys.exit(1)
...
def modify_config_file(filepath: str, settings: Dict[str, str]):
    ...
    new_lines = [line for line in lines if not line.strip().split(' ')[0] in keys_to_set]
...
@click.option(
    "--ssh",
    is_flag=True,
    help="Harden SSH server configuration"
)
@click.option(
    "--firewall",
    is_flag=True,
    help="Configure UFW firewall"
)
@click.option(
    "--install-tools",
    is_flag=True,
    help="Install recommended security tools"
)
def harden_command(
    target: Optional[str],
    all: bool,
    ...
    if not any([all, ssh, firewall, install_tools]):
        console.print("[yellow]No hardening options specified. Use --help for available options.[/yellow]")
        return
    ...
    with Progress(
        ...
        if all or ssh:
            ...
            if hardener.apply_ssh_hardening():
                progress.update(task, completed=1, description="[green]SSH hardened successfully![/green]")
            else:
        ...
        if all or firewall:
            ...
            if hardener.configure_firewall():
                progress.update(task, completed=1, description="[green]Firewall configured![/green]")
            else:
                progress.update(task, description="[red]Failed to configure firewall[/red]")
                success = False
        
        if all or install_tools:
            task = progress.add_task("Installing security tools...", total=1)
            if hardener.install_security_tools():
                progress.update(task, completed=1, description="[green]Security tools installed![/green]")
            else:
                progress.update(task, description="[red]Failed to install security tools[/red]")
                success = False
    
    if success:
        console.print("\n[bold green]Hardening completed successfully![/bold green]")
        console.print("[yellow]Note: Some changes may require a system restart to take effect.[/yellow]")
    else:
        console.print("\n[bold red]Hardening completed with errors.[/bold red]")
        console.print("Check the output above for details and ensure you have the necessary permissions.")
