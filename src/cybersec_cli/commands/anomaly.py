...
class AnomalyMonitor:
    ...
    def __init__(self, interface: str = None, use_ml: bool = True):
        self.network_detector = NetworkAnomalyDetector(
            interface=interface, use_ml=use_ml
        )
    ...
    def _generate_text_report(self) -> str:
        ...
        if self.anomalies:
            ...
            for i, anomaly in enumerate(
                sorted(self.anomalies, key=lambda x: x.score, reverse=True)[:5]
            ):
                report.append(
        ...
        report = {
            ...
            "anomalies": [
                ...
                for anomaly in sorted(
                    self.anomalies, key=lambda x: x.score, reverse=True
                )[:100]
            ],
        }
...
def display_anomalies(anomalies: List[Anomaly], max_display: int = 20) -> None:
    ...
    for i, anomaly in enumerate(sorted_anomalies[:max_display], 1):
        ...
        if isinstance(anomaly.timestamp, (int, float)):
            timestamp_str = datetime.fromtimestamp(anomaly.timestamp).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        else:
        ...
        desc = (
            ...
            if len(anomaly.description) > 80
            else anomaly.description
        )
    ...
    if len(anomalies) > max_display:
        table.caption = (
            f"... and {len(anomalies) - max_display} more anomalies not shown"
        )
...
def register_commands(cli):
    ...
    async def run_anomaly_detection(
        ...
        if not (network or logs):
            console.print(
                "[yellow]Please specify at least one monitoring option (--network/--logs)[/yellow]"
            )
            return
        ...
        try:
            with Progress(*progress_columns, console=console) as progress:
                task = progress.add_task(
                    "[cyan]Monitoring for anomalies...", total=duration, start=False
                )
                            ...
                            if anomalies:
                                ...
                                monitor.stats["network_anomalies"] += len(
                                    [
                                        a
                                        for a in anomalies
                                        if a.anomaly_type == AnomalyType.NETWORK_TRAFFIC
                                    ]
                                )
                                monitor.stats["security_alerts"] += len(
                                    [
                                        a
                                        for a in anomalies
                                        if a.anomaly_type == AnomalyType.SECURITY_ALERT
                                    ]
                                )

                                # Display new anomalies
                                if (
                                    verbose and time.time() - last_anomaly_check > 5
                                ):  # Throttle updates
                                    new_anomalies = [
                                        a for a in anomalies if a.score >= 6.0
                                    ]
                                    if new_anomalies:
    ...
    @click.option(
        "--output", "-o", type=str, help="Save report to file (supports .json, .txt)"
    )
