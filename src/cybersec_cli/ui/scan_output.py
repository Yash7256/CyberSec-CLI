...
def format_summary_panel(target: str, findings: List[Dict[str, Any]]) -> Panel:
    ...
    summary_grid.add_row(
        "🕒 [bold]Timestamp:[/]", dt.now().strftime("%Y-%m-%d %H:%M:%S")
    )
...
def format_finding_panel(finding: Dict[str, Any]) -> Panel:
    ...
    cvss_color = (
        "red" if cvss_score >= 7.0 else "yellow" if cvss_score >= 4.0 else "green"
    )
    risk_table.add_row(
        "📊 [bold]CVSS Score:[/]",
    ...
    confidence_color = (
        "green" if confidence >= 80 else "yellow" if confidence >= 50 else "red"
    )
    risk_table.add_row(
        "🎯 [bold]Confidence:[/]",
