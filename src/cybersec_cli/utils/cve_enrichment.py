...
def _coerce_to_cve_item(value: Any) -> Optional[Dict]:
    """
    Safely coerce a value to a CVE dict.
    Handles: already a dict, a JSON string, or anything else (returns None).
    """
    if isinstance(value, dict):
        if "id" in value or "cve_id" in value or "cveId" in value:
            return {
                "id": value.get("id") or value.get("cve_id") or value.get("cveId"),
                "severity": value.get("severity", "LOW"),
                "description": value.get("description", ""),
                "cvss": float(value.get("cvss", value.get("cvss_score", 0))),
            }
        return None
    ...
    if isinstance(value, str):
        try:
            ...
            if isinstance(parsed, dict):
                return _coerce_to_cve_item(parsed)
            elif isinstance(parsed, str):
                return {"id": parsed, "severity": "LOW", "description": "", "cvss": 0.0}
            return None
        except (json.JSONDecodeError, ValueError):
            if value.startswith("CVE-") or value.startswith("cve-"):
                return {"id": value.upper(), "severity": "LOW", "description": "", "cvss": 0.0}
            logger.warning("CVE enrichment: non-JSON/non-CVE string received: %.100s", value)
            return None

    logger.warning("CVE enrichment: unexpected type %s received", type(value).__name__)
    return None
...
def get_cves_for_service(
    ...
    Returns empty list if cache miss or if cached data is invalid.
    """
    ...
    if key in _cve_cache:
        ...
        if not isinstance(cached, list):
            logger.warning(
                "CVE cache for '%s' contains invalid type: %s (expected list)",
                key, type(cached).__name__
            )
            return []
        return cached

    # Check if service exists without version
    fallback_key = f"{service_name.lower()}"
    if fallback_key in _cve_cache:
        cached = _cve_cache[fallback_key]
        if not isinstance(cached, list):
            logger.warning(
                "CVE cache for '%s' contains invalid type: %s (expected list)",
                fallback_key, type(cached).__name__
            )
            return []
        return cached
...
async def enrich_service_with_live_data(
    ...
    if cached:
        # Safely coerce each cache entry to CVE dict
        valid_cves = [_coerce_to_cve_item(item) for item in cached]
        valid_cves = [cve for cve in valid_cves if cve is not None]

        if not valid_cves:
            logger.warning("CVE enrichment: cache contained no valid CVE entries for %s", service)
        else:
            cve_ids = [cve["id"] for cve in valid_cves]
            return {
                "vulnerabilities": cve_ids,
                "cvss_score": max((cve.get("cvss", 0) for cve in valid_cves), default=0),
    ...
    if live_cves:
        ...
        # Safely coerce each API entry to CVE dict
        valid_cves = [_coerce_to_cve_item(cve) for cve in live_cves]
        valid_cves = [cve for cve in valid_cves if cve is not None]
