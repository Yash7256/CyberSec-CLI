...
def create_scan_banner(scanner: PortScanner) -> Panel:
    ...
    banner_text.append(f"{scanner.target} ({scanner.ip if hasattr(scanner, 'ip') else 'N/A'})\n")
...
def format_scan_results_table(scanner: PortScanner) -> Table:
    ...
    for idx, result in enumerate(sorted(scanner.results, key=lambda x: x.port), 1):
        ...
        state = Text("✓", style="green bold") if result.state == PortState.OPEN else Text("✗", style="red")
...
def generate_security_findings(scanner: PortScanner) -> List[Panel]:
    ...
    for result in sorted(scanner.results, key=lambda x: x.port):
        ...
        if vuln_info['severity'] == Severity.INFO and not result.banner and not result.version:
            continue
        ...
        finding_text.append(f"{vuln_info['description']}", style=severity_style + " bold")
