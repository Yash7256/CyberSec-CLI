...
def create_token(
    name: Optional[str] = None, expires_in_days: int = 30
) -> Optional[str]:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            logger.warning(
                f"Token creation attempt {attempt + 1} failed due to integrity error: {e}"
            )
            if attempt < max_attempts - 1:
                continue
            else:
                logger.exception(
                    f"Failed to create token after {max_attempts} attempts: {e}"
                )
                return None
