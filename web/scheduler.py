...
def toggle_scheduled_scan(scan_id: int, enabled: bool) -> bool:
    ...
    try:
        conn = sqlite3.connect(SCHEDULER_DB)
        c = conn.cursor()
        c.execute('UPDATE scheduled_scans SET enabled = ? WHERE id = ?', (enabled, scan_id))
        conn.commit()
        conn.close()
...
def _add_job_to_scheduler(scan_id: int, target: str, cron_expression: str):
    ...
    async def _run_scan():
        try:
            ...
            c = conn.cursor()
            now = datetime.utcnow().isoformat() + 'Z'
            c.execute('UPDATE scheduled_scans SET last_run = ? WHERE id = ?', (now, scan_id))
            conn.commit()
            conn.close()
            ...
            if process.returncode == 0:
                ...
                logger.warning(f'Scheduled scan {scan_id} failed with code {process.returncode}')
        except Exception as e:
...
async def init_scheduler():
    ...
    try:
        ...
        for scan in scans:
            if scan['enabled']:
                _add_job_to_scheduler(scan['id'], scan['target'], scan['cron_expression'])
        
        logger.info(f'Loaded {len([s for s in scans if s["enabled"]])} enabled scheduled scans')
    except Exception as e:
