...
async def demonstrate_postgresql_usage():
    ...
    try:
        ...
        scan_id = await db_interface.create_scan(
            target="example.com",
            user_id="user_123",
            config={"scan_type": "TCP", "ports": "1-1000", "timeout": 1.0},
        )
        ...
        sample_results = [
            ...
            },
            {
                ...
                "risk": "medium",
                "metadata": {"cve": ["CVE-2021-41773"], "confidence": 0.90},
            },
            {
                "port": 443,
                "state": "open",
        ...
        for result in results:
            print(
                f"     Port {result['port']}: {result['service']} ({result['state']})"
            )
        print()
