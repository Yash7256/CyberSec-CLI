...
class TestClientLimits:
    ...
    def test_client_within_limit(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'zcard', return_value=5), \
             patch.object(rate_limiter.redis, 'zremrangebyscore'), \
             patch.object(rate_limiter.redis, 'zadd'), \
             patch.object(rate_limiter.redis, 'expire'):
    ...
    def test_client_exceeds_limit(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'zcard', return_value=15), \
             patch.object(rate_limiter.redis, 'zremrangebyscore'):
            result = rate_limiter.check_client_limit(client_id)
...
class TestTargetLimits:
    ...
    def test_target_within_limit(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'zcard', return_value=50), \
             patch.object(rate_limiter.redis, 'zremrangebyscore'), \
             patch.object(rate_limiter.redis, 'zadd'), \
             patch.object(rate_limiter.redis, 'expire'):
    ...
    def test_target_exceeds_limit(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'zcard', return_value=150), \
             patch.object(rate_limiter.redis, 'zremrangebyscore'):
            result = rate_limiter.check_target_limit(target)
            assert result is False
...
class TestGlobalConcurrentLimits:
    ...
    def test_increment_concurrent_scan(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'incr', return_value=100), \
             patch.object(rate_limiter.redis, 'expire'):
    ...
    def test_increment_concurrent_scan_exceeds(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'incr', return_value=1500), \
             patch.object(rate_limiter.redis, 'expire'):
            result = rate_limiter.increment_concurrent_scan()
...
class TestExponentialBackoff:
    ...
    def test_violation_recording(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'get', return_value='0'), \
             patch.object(rate_limiter.redis, 'incr', return_value=1):
    ...
    def test_cooldown_application(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'get', return_value='2'), \
             patch.object(rate_limiter.redis, 'setex'):
...
class TestRateLimitMetrics:
    ...
    def test_get_all_violations(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'keys', return_value=[b'rate_limit:violations:client1', b'rate_limit:violations:client2']), \
...
class TestRateLimitReset:
    ...
    def test_reset_client_limits(self, rate_limiter):
        ...
        with patch.object(rate_limiter.redis, 'delete') as mock_delete:
            ...
            assert mock_delete.call_count >= 3  # At least client, violation, and cooldown keys