...
class TestCacheHitMissLogic:
    ...
    async def test_cache_store(self, mock_redis_client):
        ...
        with patch('core.scan_cache.redis_client', mock_redis_client):
            cache_key = cache.get_cache_key('192.168.1.1', [22, 80])
            success = await cache.store_cache(cache_key, scan_result, target='192.168.1.1')
            
            assert success is True
            assert mock_redis_client.set.called  # Check that set was called at least once
    ...
    async def test_cache_store_with_compression(self, mock_redis_client):
        ...
        with patch('core.scan_cache.redis_client', mock_redis_client):
            cache_key = cache.get_cache_key('192.168.1.1', [22, 80])
            success = await cache.store_cache(cache_key, large_result, target='192.168.1.1')
...
class TestTTLExpiration:
    ...
    async def test_cache_ttl_set_correctly(self, mock_redis_client):
        ...
        with patch('core.scan_cache.redis_client', mock_redis_client):
            cache_key = cache.get_cache_key('192.168.1.1', [22, 80])
            await cache.store_cache(cache_key, scan_result, ttl=expected_ttl, target='192.168.1.1')
            
            # Check that set was called with the correct TTL
            assert mock_redis_client.set.called
            call_args = mock_redis_client.set.call_args
            if call_args and len(call_args[1]) >= 1:  # Use kwargs for TTL
                ttl_value = call_args[1]['ttl'] if 'ttl' in call_args[1] else (call_args[0][2] if len(call_args[0]) > 2 else None)
    ...
    async def test_cache_ttl_custom(self, mock_redis_client):
        ...
        with patch('core.scan_cache.redis_client', mock_redis_client):
            cache_key = cache.get_cache_key('192.168.1.1', [22, 80])
            await cache.store_cache(cache_key, scan_result, ttl=custom_ttl, target='192.168.1.1')
...
class TestCacheInvalidate:
    ...
    async def test_invalidate_specific_key(self, mock_redis_client):
        ...
        with patch('core.scan_cache.redis_client', mock_redis_client):
            ...
            assert result is True  # We expect the return value to be True based on the Redis result