...
class ScannerBenchmark:
    ...
    async def benchmark_memory_usage_during_large_scan(self) -> Dict:
        ...
        tracking_thread = threading.Thread(
            target=lambda: [time_module.sleep(0.1) or track_memory() for _ in range(50)]
        )
    ...
    async def run_all_benchmarks(self) -> Dict:
        ...
        print(
            f"Concurrent scans throughput: {results['concurrent_scans']['throughput']:.2f} scans/sec"
        )

        results["cache_performance"] = await self.benchmark_cache_hit_rate()
        print(
            f"Cache hit duration: {results['cache_performance']['cache_hit_duration']:.4f}s"
        )

        results["memory_usage_large_scan"] = (
            await self.benchmark_memory_usage_during_large_scan()
        )
        print(
            f"Large scan duration: {results['memory_usage_large_scan']['duration']:.2f}s"
        )

        results["adaptive_scanning"] = await self.benchmark_adaptive_scanning()
        print(
            f"Adaptive scanning overhead: {results['adaptive_scanning']['adaptive_overhead']:.6f}s"
        )
...
async def main():
    ...
    print(
        "Benchmark completed. Results saved to tests/performance/scanner_benchmark_report.txt"
    )
