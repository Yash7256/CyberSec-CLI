...
class ScanTasks(TaskSet):
    ...
    def get_status(self):
        ...
        with self.client.get(
            "/api/status", headers=self.headers, catch_response=True
        ) as response:
    ...
    def health_check(self):
        ...
        with self.client.get(
            "/health", headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def redis_health_check(self):
        ...
        with self.client.get(
            "/health/redis", headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def scan_endpoint(self):
        ...
        target = random.choice(
            targets[:3]
        )  # Exclude example.com to avoid validation issues

        # Prepare scan parameters
        params = {"ports": "1-100", "timeout": 0.5, "max_concurrent": 5}
        ...
        with self.client.get(
            ...
            if response.status_code in [
                200,
                429,
            ]:  # 429 is rate limit, which is expected under load
    ...
    def get_metrics(self):
        ...
        with self.client.get(
            "/metrics", headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
...
class RateLimitTestUser(HttpUser):
    ...
    def on_start(self):
        ...
        self.headers = {"Content-Type": "application/json", "X-API-Key": "test-api-key"}
