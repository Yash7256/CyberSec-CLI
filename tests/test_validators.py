...
from core.validators import (
    validate_target, validate_port_range, 
    sanitize_input, is_safe_path, validate_file_path, validate_url
)
...
class TestTargetValidation:
    ...
    def test_invalid_ipv4_addresses(self):
        ...
        for ip in invalid_ips:
            ...
            assert isinstance(result, bool), f"validate_target should return boolean for {ip}"
    ...
    def test_invalid_domains(self):
        ...
        for domain in invalid_domains:
            ...
            assert isinstance(result, bool), f"validate_target should return boolean for {domain}"
    ...
    def test_special_blocked_domains(self):
        ...
        for domain in blocked_domains:
            assert validate_target(domain) is False, f"Blocked domain {domain} should be rejected"
...
class TestPortRangeValidation:
    ...
    def test_valid_port_ranges(self):
        ...
        for port_list in valid_ranges:
            ...
            assert result is True, f"Port list {port_list} should be valid, got {result}"
    ...
    def test_invalid_port_ranges(self):
        ...
        for port_list in invalid_ranges:
            result = validate_port_range(port_list)
            assert result is False, f"Invalid port list {port_list} should be rejected, got {result}"
    ...
    def test_port_range_type_validation(self):
        ...
        for invalid_input in invalid_types:
            ...
            assert result is False, f"Invalid type {type(invalid_input)} should be rejected"
...
class TestInputSanitization:
    ...
    def test_input_sanitization_basic(self):
        ...
        for input_val, expected in test_cases:
            ...
            assert result == expected, f"Sanitization of '{input_val}' should result in '{expected}', got '{result}'"
    ...
    def test_safe_paths(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for path, base_path in safe_paths:
                os.makedirs(os.path.dirname(path), exist_ok=True) if os.path.dirname(path) != temp_dir else None
                assert is_safe_path(path, base_path) is True, f"Path {path} should be safe within {base_path}"
    ...
    def test_unsafe_paths(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for path, base_path in unsafe_paths:
                assert is_safe_path(path, base_path) is False, f"Path {path} should be unsafe within {base_path}"
...
class TestFilePathValidation:
    ...
    def test_file_extension_validation(self):
        ...
        for file in valid_files:
            assert validate_file_path(file, allowed_extensions) is True, f"File {file} should be valid with allowed extensions"
        
        # Invalid files should be rejected with allowed extensions
        for file in invalid_files:
            assert validate_file_path(file, allowed_extensions) is False, f"File {file} should be invalid with allowed extensions"
        ...
        for file in valid_files + invalid_files:
            if '../' not in file and './' not in file:
                ...
                assert result is True, f"File {file} should be valid without extension filter"
...
class TestEdgeCases:
    ...
    def test_none_inputs(self):
        ...
        for func, desc in none_inputs:
            result = func()
            assert isinstance(result, bool), f"{desc} should return boolean, got {type(result)}"
        
        # Special case for sanitize_input which returns a string
        result = sanitize_input(None)
        assert isinstance(result, str), f"sanitize_input with None should return string, got {type(result)}"