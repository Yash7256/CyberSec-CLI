...
async def test_scan_cache():
    ...
    try:
        ...
        store_result = await scan_cache.store_cache(cache_key, test_data, target="example.com")
        if store_result:
...
async def test_scan_with_caching():
    ...
    try:
        ...
        if first_scan_time > 0:
            speedup = first_scan_time / max(second_scan_time, 0.001)  # Avoid division by zero
