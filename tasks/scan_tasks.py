...
# Import metrics
try:
    from monitoring.metrics import metrics_collector, record_scan, start_timer, stop_timer
    HAS_METRICS = True
except ImportError:
    HAS_METRICS = False
...
def perform_scan_task(self, scan_id: str, target: str, ports: str = "1-1000", 
    ...
    try:
        ...
        if HAS_SCAN_CACHE and scan_cache and not force_scan:
            ...
            if cached_result:
                ...
                if HAS_METRICS:
                    ...
                    metrics_collector.observe_open_ports_found(len(cached_result.get('results', [])))
                    metrics_collector.increment_scan(status="completed", user_type="unknown")
                ...
                if HAS_DB_MODULES:
                    try:
                        ...
                        logger.info(f"Saved cached scan results for {scan_id} to database")
                    except Exception as e:
                        logger.error(f"Failed to save cached scan results to database: {e}")
                
                logger.info(f"Completed scan task {scan_id} for target {target} (cached)")
                return result_data
        
        # Update task state to show we're starting
        self.update_state(state='PROGRESS', meta={'status': 'Starting scan...', 'progress': 0})
        ...
        for i, group in enumerate(priority_groups):
            ...
            self.update_state(state='PROGRESS', meta={
                ...
                'progress': round((scanned_ports / total_ports) * 100) if total_ports > 0 else 0,
            ...
            self.update_state(state='PROGRESS', meta={
                ...
                'open_ports_found': len([r for r in results if r.state == PortState.OPEN]),
        ...
        if HAS_METRICS:
            scan_duration = stop_timer(scan_start_time)
            record_scan(scan_start_time, status="completed", user_type="unknown", 
                       ports_count=total_ports, open_ports_count=len(all_results))
        ...
        if HAS_METRICS:
            scan_duration = stop_timer(scan_start_time)
            metrics_collector.increment_scan_error(error_type=str(type(exc).__name__), target_type="unknown")
            metrics_collector.observe_scan_duration(scan_duration)
