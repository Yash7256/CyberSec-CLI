...
class SmartRateLimiter:
    ...
    def check_port_range_limit(self, ports: List[int]) -> tuple[bool, str]:
        ...
        if port_count > self.port_limit:
            return (
                False,
                f"Port range too large: {port_count} ports (max {self.port_limit})",
            )
        elif port_count > self.warn_port_threshold:
            return (
                True,
                f"Warning: {port_count} ports selected (suggestion: use fewer ports for better performance)",
            )
    ...
    def record_violation(self, client_id: str) -> int:
        ...
        # Log the violation
        logger.warning(
            f"Rate limit violation for client {client_id}, violation #{violation_count}"
        )
