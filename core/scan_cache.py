...
class ScanCache:
    ...
    def _get_default_ttl(self, target: str) -> int:
        ...
        try:
            ...
            if any(internal_domain in target.lower() for internal_domain in 
                   ['internal', 'local', 'intranet', 'corp', 'company']):
    ...
    async def store_cache(self, cache_key: str, results: Dict[str, Any], 
                         ttl: Optional[int] = None, target: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if len(json_data) > 1024:  # Compress if larger than 1KB
                ...
                result = redis_client.set(cache_key, compressed_data, 
                                        ttl or (self._get_default_ttl(target) if target else 3600))
            else:
                result = redis_client.set(cache_key, json_data, 
                                        ttl or (self._get_default_ttl(target) if target else 3600))
