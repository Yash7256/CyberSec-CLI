...
SERVICE_PROBES = {
    ...
    "ssh": [
        b""  # SSH version exchange happens automatically on connection
    ],
    "ftp": [
        b""
    ],
    ...
    "mysql": [
        b"\x0a\x00\x00\x01\x85\xa6\x3f\x20\x00\x00\x00\x01\x21"
    ],
    "postgresql": [
...
async def send_async_probe(ip: str, port: int, probe_data: bytes, timeout: float = 3.0) -> bytes:
    """
...
async def identify_service_async(ip: str, port: int, timeout: float = 3.0) -> Dict[str, Any]:
    """
